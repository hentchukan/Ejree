...
import prv.adt.ejree.data.database.BestPerformanceDAO;
...
public class ShowBestPerformance extends Fragment {
	...
	private BestPerformanceDAO dao;
	private int[] heads = {R.id.show_best_performance_periode, R.id.show_best_performance_distance, R.id.show_best_performance_speed, R.id.show_best_performance_time};
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		View rootView = inflater.inflate(R.layout.show_best_performances, container, false);
		...
		RadioButton rb = (RadioButton) rootView.findViewById(R.id.show_best_performance_month);
		rb.setSelected(unit == Calendar.DAY_OF_WEEK);
		rb.setOnClickListener(new CheckRadioListener());
		rb = (RadioButton) rootView.findViewById(R.id.show_best_performance_week);
		rb.setSelected(unit == Calendar.DAY_OF_MONTH);
		rb.setOnClickListener(new CheckRadioListener());
		rb = (RadioButton) rootView.findViewById(R.id.show_best_performance_year);
		rb.setSelected(unit == Calendar.DAY_OF_YEAR);
		rb.setOnClickListener(new CheckRadioListener());
		
		Button born = (Button) rootView.findViewById(R.id.show_best_performance_bornInf);
		born.setText(DateHelper.getFormattedDate(bornInf));
		born.setOnClickListener(new DatePickerListener());
		
		born = (Button) rootView.findViewById(R.id.show_best_performance_bornSup);
		born.setText(DateHelper.getFormattedDate(bornSup));
		born.setOnClickListener(new DatePickerListener());
		
		ImageView search = (ImageView) rootView.findViewById(R.id.show_best_performance_search);
		...
		listView = (ListView) rootView.findViewById(R.id.show_best_performance_list);
		FrequencyListAdapter adapter = new FrequencyListAdapter(getActivity(), frequencies);
		listView.setAdapter(adapter);
		dao = new BestPerformanceDAO(getActivity());
	    dao.open();
	...
	protected void search() {
		...
		for (Periode periode : periodes) {
			...
			try {
				List<Frequency> results = dao.retrieveSql("Select count(*) as RUNS_NUMBER, max(RUN_DISTANCE) as RUNS_DISTANCE, max(RUN_TIME) as RUNS_TIME, max(RUN_CALORIS) as RUNS_CALORIS, max(RUN_TIME/RUN_DISTANCE) as RUNS_SPEED from RUN Where strftime('%Y-%m-%d', RUN_DATE) BETWEEN '"+sdf.format(periode.getBegin())+"' AND '"+sdf.format(periode.getEnd())+"'", null);
		...
		Collections.sort(frequencies, new Comparator<Frequency>() {
			...
			public int compare(Frequency lhs, Frequency rhs) {
				if (lhs != null && rhs != null && lhs.getRuns() != null && rhs.getRuns() != null)
					return lhs.getRuns().compareTo(rhs.getRuns());
				else return Integer.valueOf(lhs.hashCode()).compareTo(rhs.hashCode());
			}
		});
	}
	...
	private class DatePickerListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if (v.getId() == R.id.show_best_performance_bornInf)
	...
	private class CheckRadioListener implements OnClickListener {
		@Override
		public void onClick(View v) {
			if (v.getId() == R.id.show_best_performance_week )
				unit = Calendar.DAY_OF_WEEK;
			if (v.getId() == R.id.show_best_performance_month)
				unit = Calendar.DAY_OF_MONTH;
			if (v.getId() == R.id.show_best_performance_year)
	...
	private class DatePickerDialogListener implements OnDateSetListener {
		...
		public void onDateSet(DatePicker view, int year, int monthOfYear, int dayOfMonth) {
			...
			if (id == R.id.show_best_performance_bornInf) {
